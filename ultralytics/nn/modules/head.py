...
class CompositeDetect(nn.Module):
    ...
    def forward(self, x):
        ...
        pred = []
        for i in range(self.nl):
            box = self.cv2[i](x[i])  # (bs, 4*reg_max, H, W)
            cls = self.cv3[i](x[i])  # (bs, nc, H, W)
            pred.append(torch.cat((box, cls), 1))  # (bs, no, H, W)

        if self.training:
            # Run auxiliary heads, but DO NOT return them
            for i in range(self.nl, 2 * self.nl):
                _ = self.cv4[i - self.nl](x[i])
                _ = self.cv5[i - self.nl](x[i])
            return pred  # ✅ Return just the 3 main outputs for the loss
            ...
            if hasattr(self, 'dfl_aux'):
                for i in range(self.nl, 2 * self.nl):
                    _ = self.cv4[i - self.nl](x[i])
                    _ = self.cv5[i - self.nl](x[i])
            self.anchors, self.strides = (x.transpose(0, 1) for x in make_anchors(pred, self.stride, 0.5))
            self.shape = shape

        # Decode
        pred = torch.cat([p.view(bs, self.no, -1) for p in pred], dim=2)
